    ...
    if (searchValue.length > 0) {
        ...
        setTimeout(() => {
            if (firstVisibleItem) {
                ...
                setTimeout(() => { firstVisibleItem.style.boxShadow = ''; }, 2000);
            }
        }, 500);
        ...
        if (visibleCount === 0) {
            ...
            message.style.cssText = `text-align:center;padding:3rem 2rem;color:var(--gray-500);grid-column:1/-1;`;
            message.innerHTML = `<i class="fas fa-search" style="font-size:3rem;margin-bottom:1rem;display:block;opacity:0.5;"></i>
...
function highlightText(element, searchTerm) {
    ...
    nodes.forEach(node => {
        ...
        if (index !== -1) {
            ...
            span.style.cssText = `background:linear-gradient(120deg, rgba(52,165,120,0.3) 0%, rgba(52,165,120,0.1) 100%);padding:0.1rem 0.3rem;border-radius:3px;font-weight:600;color:var(--primary);`;
...
function filterKategori(kategori, btn) {
    ...
    if (visibleCount === 0 && kategori !== 'all') {
        if (!noResultMsg) {
            ...
            noResultMsg.style.cssText = `text-align:center;padding:2rem;color:var(--gray-500);grid-column:1/-1;background:var(--gray-50);border-radius:var(--radius-md);margin-top:1rem;`;
            document.getElementById('arsipList').appendChild(noResultMsg);
        }
        noResultMsg.innerHTML = `<i class="fas fa-folder-open" style="font-size:2rem;margin-bottom:0.5rem;display:block;"></i><p>Belum ada dokumen untuk kategori "${kategori}"</p>`;
    } else if (noResultMsg) {
        noResultMsg.remove();
    }
...
async function loadPengumuman() {
    ...
    try {
        ...
        if (pengumumanAktif.length === 0) {
            grid.innerHTML = `<div style="grid-column: 1/-1; text-align: center; padding: 2rem; color: var(--gray-500);">
                <i class="fas fa-bell-slash" style="font-size: 2rem; margin-bottom: 0.5rem; display: block;"></i>
                Belum ada pengumuman saat ini</div>`;
            return;
        }
        const prioritasOrder = { tinggi: 1, sedang: 2, rendah: 3 };
        pengumumanAktif.sort((a, b) => (prioritasOrder[a.prioritas] || 3) - (prioritasOrder[b.prioritas] || 3));
        grid.innerHTML = pengumumanAktif.map(p => `
            <a href="${p.link || 'https://ustjogja.ac.id/id/berita/'}" target="_blank" class="pengumuman-card prioritas-${p.prioritas || 'sedang'}">
                <div class="pengumuman-header">
                    ...
                    <span class="pengumuman-tanggal"><i class="far fa-calendar"></i> ${p.tanggal}</span>
                </div>
                <h3 class="pengumuman-judul">${p.judul}</h3>
                <p class="pengumuman-deskripsi">${p.deskripsi}</p>
                <div class="pengumuman-footer">
                    <span class="pengumuman-baca">Baca Selengkapnya <i class="fas fa-arrow-right"></i></span>
...
// ==================== EVENT LOAD (DIPINDAHKAN KE PALING AKHIR) ====================
window.addEventListener('load', function() {
    // ===== MUAT AGENDA DARI JSON TERLEBIH DAHULU =====
    if (typeof window.loadAgendaFromJSON === 'function') {
        window.loadAgendaFromJSON();
